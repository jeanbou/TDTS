function m_ratio = m_Hellinger_Distance(X,T);
...
for m_Class1 = min(T) : (max(T)-1)
    for m_Class2 = (m_Class1+1) : max(T)  %%% create all possible pair comparising
        ...
        for i = 1 : size(pureXsort,2)            
            t_divergence{m_genIndex,i} =  (sqrt(PxC1(i))-sqrt(PxC2(i)))^2;            
        end;
...
for j = 1 : size(t_divergence,1) 
    ...
    m_t_divergence(j) = sqrt(m_int_sum/2);
end;

m_ratio = min(m_t_divergence);
return;