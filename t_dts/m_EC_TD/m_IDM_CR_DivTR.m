function m_ratio = m_IDM_CR_DivTR(X,T,err_ZeroDet);
% --------------------------------------------------------------
% Interclass Distance Measure, Criterion J4 (ref on Fukunaga) 
...
% Output:
% - m_ration
% Copyright by Ivan Budnyk 2008 / the 11th of March
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for m_classIndex = m_minClassNumber:m_maxClassNumber,
	for m_classIndex2 = (m_classIndex+1):m_maxClassNumber,
       ...
       if (size(indexes1) ~= 0) & (size(indexes2) ~= 0)  %%% This check allows to have holes between class Indexation, for example T = [1 3 4], this procedure is working when no class #2
               ...
               if ( abs(trace(G)) > err_ZeroDet )
                  m_indexFDR = m_indexFDR + 1;
                  m_multiClassFDRVector(m_indexFDR) = trace(GB1)/trace(G); %%% Normalazing by the principle 1-zero)
               else
                   fprintf('m_IDM_CR_DivTR :: Warning(1) :: Matrix cannot be inverted, problem is simple, assigning complexity ratio skipped\n');
               end;
