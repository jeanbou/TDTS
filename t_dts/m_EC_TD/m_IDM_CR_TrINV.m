function m_ratio = m_IDM_CR_TrINV(X,T,err_ZeroDet);
% --------------------------------------------------------------
% Interclass Distance Measure, Criterion J1 (ref on Fukuunaga) 
...
for m_classIndex = m_minClassNumber:m_maxClassNumber,
	for m_classIndex2 = (m_classIndex+1):m_maxClassNumber,
       ...
       if (size(indexes1) ~= 0) & (size(indexes2) ~= 0)  %%% This check allows to have holes between class Indexation, for example T = [1 3 4], this procedure is working when no class #2
               ...
               if (abs(det(G)) > err_ZeroDet)
                  m_indexFDR = m_indexFDR + 1;
                  m_multiClassFDRVector(m_indexFDR) = trace(GB1*inv(G));
               else
                  fprintf('m_IDM_CR_TrINV :: Warning(1.1) :: Global matrix cannot be inverted, trying to use pseudoinverted matrix\n');
                  if (abs(det(G'*G)) > err_ZeroDet)
                        pseudoG = inv(G'*G)*G';
                        m_indexFDR = m_indexFDR + 1;
                        m_multiClassFDRVector(m_indexFDR) = trace(GB1*inv(pseudoG));
                  else
                        fprintf('m_IDM_CR_TrINV :: Warning(1.2) :: Pseudoinverted matrix cannot be inverted, problem is simple, assigning complexity ratio skipped\n');
                  end;
               end;              
