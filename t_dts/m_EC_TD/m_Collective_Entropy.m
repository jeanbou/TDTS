function m_ratio = m_Collective_Entropy(X,T,B);
% --------------------------------------------------------------
...
for i = 1:size(X,1)
    if (array_ofBDeltas(i) ~= 0 )
        for res = 1:B
            ...
            for j = min(T):max(T)
                if (Pil(i,res,j) ~= 0) 
                    El(i,res) = El(i,res) - Pil(i,res,j)*log(Pil(i,res,j));
                end;
            end;
        end;
    end;
...
for i = 1:size(X,1)
    ...
    if (array_ofBDeltas(i) ~= 0 )
        for res = 1:B
            ...
            E(i) = E(i) + El(i,res) * SumN;
        end;
    end;
end;
clear('El','Pil');
Ec = (1 - E)/(2^(B-1));  %%% "this is to ge a larger weight for higher resolution" // the citation of article of Singh for 2004
clear('E');
